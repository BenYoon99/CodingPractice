...
SET @seed = 111;
### Mysql @seed 구글링 해볼것
### 세션은 사용자가 MySQL 서버에 연결하고 있는 동안의 작업 단위를 의미한다. 세션이 끝난다는 것은 사용자의 MySQL 연결이 종료되거나 닫힐 때를 말한다.
UPDATE article a
...
SELECT 
    ...
    A.regDate AS '작성일'
FROM article A
INNER JOIN `member` M ON A.mno = M.mno
INNER JOIN board B ON A.bno = B.bno
ORDER BY A.ano DESC;
...
INNER JOIN `member` M ON A.mno = M.mno
INNER JOIN board B ON A.bno = B.bno
LEFT JOIN articleReply R ON A.ano = R.ano 
GROUP BY A.ano 
ORDER BY `댓글수` DESC, A.ano DESC; 


# 모든 게시물의 개수 출력
SELECT COUNT(*) AS '총 게시물 수' FROM article;


# 모든 게시물의 조회수의 총합 출력
SELECT SUM(hit) AS '총 조회수 합' FROM article;


# 이순신이 작성한 댓글의 내용, 작성자 닉네임, 작성일 출력. 작성일로 내림차순
SELECT R.rbody, M.nname, R.regDate
FROM articleReply R
...
SELECT B.bname, COUNT(A.ano) AS '게시물수'
FROM board B
LEFT JOIN article A ON B.bno = A.bno
GROUP BY B.bno
ORDER BY `게시물수` DESC;
...
SELECT YEAR(regDate) AS '연도', COUNT(*) AS '개수'
FROM article
...
SELECT B.bname, COUNT(R.arno) AS '댓글수'
FROM board B
INNER JOIN article A ON B.bno = A.bno
INNER JOIN articleReply R ON A.ano = R.ano
GROUP BY B.bno
ORDER BY `댓글수` DESC
LIMIT 1;
...
SELECT B.bname, A.title, M.nname, A.regDate, A.hit, 
       COUNT(DISTINCT L.mno) AS '좋아요수', 
       COUNT(DISTINCT R.arno) AS '댓글수'
FROM article A
INNER JOIN board B ON A.bno = B.bno
INNER JOIN `member` M ON A.mno = M.mno
LEFT JOIN `like` L ON A.ano = L.ano
LEFT JOIN articleReply R ON A.ano = R.ano
GROUP BY A.ano;
